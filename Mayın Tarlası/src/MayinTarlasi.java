...
public class MayinTarlasi implements MouseListener {
	...
	public MayinTarlasi() {
		...
		frame.setLayout(new GridLayout(10, 10));// 10'a 10'luk alan yaptık 
	...
	public void generateMine() { //Rastgele mayınlar üretilir
		int i = 0;
		while (i < 10) {
			int randRow = (int) (Math.random() * board.length);
			int randCol = (int) (Math.random() * board[0].length);

			while (board[randRow][randCol].isMine()) {//aynı yere mayın koymamak icin kontrol ettik
				randRow = (int) (Math.random() * board.length);
				randCol = (int) (Math.random() * board[0].length);
			}
			board[randRow][randCol].setMine(true);//rastgele mayınları yerleştridik
			i++;
		}
	}
		...
		for (int row = 0; row < board.length; row++) {
			for (int col = 0; col < board[0].length; col++) {
				if (board[row][col].isMine()) {
					board[row][col].setIcon(new ImageIcon("Image/mine.png"));
				} else {
	...
	public void updateCount() {
		for (int row = 0; row < board.length; row++) {//mayınları gezmek icin döngü kullandık
			for (int col = 0; col < board[0].length; col++) {
				if (board[row][col].isMine()) {
					counting(row, col);
				}
	...
	public void counting(int row, int col) {//mayınların etrafını sayacak
		for (int i = row - 1; i <= row + 1; i++) {
			for (int k = col - 1; k <= col + 1; k++) {
				try {//sınırların dışına çıkmamak icin bunu yaptık 
	...
	public void open(int r, int c) {//mayın olmayan alanları açmak için 
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getButton() == 1) {
			...
			if (!b.isFlag()) {
				b.setIcon(new ImageIcon("Image/flag.png"));
				b.setFlag(true);
			} else {
				b.setIcon(null);
				b.setFlag(false);
